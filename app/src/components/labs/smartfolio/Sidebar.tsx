...
import { ACCOUNTS, type AccountId } from '@/lib/smartfolio/store/portfolio';
        ...
        <>
            ...
            `}>
                ...
                <div className="px-4 pb-4 space-y-1.5">
                    ...
                    <Link to={`${basePath}/aum`} onClick={() => setIsOpen(false)}
                ...
                <div className="px-6 pb-6 pt-2">
                    ...
                    {/* Market Signal Tile */}
                    <Link to={`${basePath}/market`} className="block mt-2 p-3 rounded-xl bg-white/[0.02] border border-white/5 hover:bg-white/[0.05] transition-all group">
...
const NavItem = ({ icon, label, href, active, onClick }: { icon: string; label: string; href: string; active?: boolean; onClick?: () => void }) => (
    <Link to={href} onClick={onClick} className={`flex items-center gap-3 px-4 py-2 rounded-lg transition-colors ${active ? 'bg-white/5 text-white' : 'text-gray-400 hover:bg-white/5'}`}>
