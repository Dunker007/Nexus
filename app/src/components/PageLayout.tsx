...
export function StatPill({ label, value, icon, color = 'cyan', pulse = false, className = '' }: StatPillProps) {
  return (
    <div className={`flex items-center gap-2 px-3 py-1.5 border rounded-lg ${colorMap[color]} ${className}`}>
      ...
      {value !== undefined && <span className="text-[10px] font-black uppercase tracking-widest">{value}</span>}
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          {subtitle && <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30 mt-1">{subtitle}</p>}
        </div>
      </div>
...
export default function PageLayout({
  ...
  return (
    <div className={`w-full min-h-full relative overflow-hidden text-white ${className}`}>
      ...
      <div
         className={`relative z-10 w-full ${noPadding ? 'h-full overflow-hidden' : 'py-8 px-6 max-w-[1600px] mx-auto'} ${containerClass}`}
      >
        {children}
      </div>
    </div>
