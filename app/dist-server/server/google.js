...
export const resolveFolderId = (id) => {
    let folderId = id || process.env.GDRIVE_FOLDER_ID || 'root';
    if (folderId)
        folderId = folderId.replace(/['"]/g, '').trim().split('?')[0];
    if (!folderId || folderId === '.')
        return 'root';
    if (folderId !== 'root' && !DRIVE_ID_RE.test(folderId))
        return 'root';
    return folderId;
};
export const resolveOpsFileId = () => {
    ...
    if (!fileId)
        return undefined;
    fileId = fileId.trim();
    if (['auto', 'none', 'null', 'false', 'create', ''].includes(fileId.toLowerCase()))
        return undefined;
