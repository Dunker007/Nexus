...
newsRouter.post('/refresh', requireAuth, async (req, res) => {
    ...
    const results = await Promise.allSettled(allFeeds.map(source => parser.parseURL(source.rss).then(feedData => ({ source, feedData }))));
    let addedCount = 0;
                ...
                if (!existing) {
                    await getPrisma().news_items.create({
                        data: {
                            ...
                            impact: source.logo,
                            feed: source.priority ? 'priority' : 'nexus',
                            image
                        }
                    });
                    addedCount++;
