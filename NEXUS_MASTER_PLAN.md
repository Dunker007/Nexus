...
User Query
    ↓
Lux (Gemma 4 @ LM Studio) ← Tool Registry
    ↓
    ├── Local tools (weather, time, system)
    ├── delegate_to_agent → Newsician, CodeAgent, etc.
    ├── query_memory → Pieces OS LTM
    └── upgrade_model → OpenRouter (free/cheap → costly)
    ↓
Consolidated Output
```
...
- Add real `get_weather` tool (Open-Meteo free API, no key required)
- Add `get_system_time` tool from shared registry
- Test: Gemma 4 returns time AND weather in one consolidated response

**Phase O-2: Agent Delegation**
